...
#include "test/speed_test_inl/setup.inl"
    ...
    vector<PI_float_add_intermediate> intermediate_vec;
...
#pragma omp parallel for
    for (int i = 0; i < vec_len / batch_size; i++) {
        ...
        for (int vi = i * batch_size; vi < min((i + 1) * batch_size, vec_len); vi++) {
            ...
            PI_float_add_intermediate &intermediate = intermediate_vec[vi];
            FLTshare_preprocess(0, party_id, PRGs, *netio, &x_share);
            FLTshare_preprocess(0, party_id, PRGs, *netio, &y_share);
            PI_float_add_preprocess(party_id, PRGs, *netio, PRGs[2], intermediate, &x_share,
                                    &y_share, &z_share);
        }
        if (party_id == 0) {
...
#include "test/speed_test_inl/after_preprocess.inl"
    ...
    vector<vector<PI_float_add_intermediate *>> intermediate_batch_vec(vec_len / batch_size + 1);
    for (int i = 0; i < vec_len / batch_size; i++) {
...
#pragma omp parallel for
    for (int i = 0; i < vec_len / batch_size; i++) {
        ...
        NetIOMP *netio = netio_list[thread_id];
        vector<PRGSync> &PRGs = PRGs_list[thread_id];
        PI_float_add_vec(party_id, PRGs, *netio, intermediate_batch_vec[i], x_share_batch_vec[i],
